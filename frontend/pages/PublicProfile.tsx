...
const PublicProfile = () => {
  ...
  const loadUser = async () => {
    try {
      const response = await axios.get(`https://codeclash-2-g5un.onrender.com/api/users/${userId}`, {
  ...
  const loadFriendshipStatus = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://codeclash-2-g5un.onrender.com/api/friends/search?query=${userId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
